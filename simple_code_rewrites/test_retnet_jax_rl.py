...
def create_decay_matrix_rl_jax(gamma: float, size: int, dones_in_chunk: jnp.ndarray) -> jnp.ndarray:
    """
...
def create_cross_chunk_mask_rl_jax(size: int, dones_in_chunk: jnp.ndarray) -> jnp.ndarray:
    """
...
def recurrent_retention_rl_jax(Q: jnp.ndarray, K: jnp.ndarray, V: jnp.ndarray, dones: jnp.ndarray, gamma: float, W_g: jnp.ndarray, W_o: jnp.ndarray) -> jnp.ndarray:
    """
    ...
    inputs = (Q, K_scaled, V, dones_prev)
    ...
    def scan_body_recurrent_rl(carry_S, x_t):
        ...
        new_S = jnp.exp(gamma) * S_reset_or_not + jnp.outer(k_n_scaled, v_n)
        ret_n = q_n @ new_S
        return new_S, ret_n
...
def chunkwise_retention_rl_jax(Q: jnp.ndarray, K: jnp.ndarray, V: jnp.ndarray, dones: jnp.ndarray, gamma: float, chunk_size: int, W_g: jnp.ndarray, W_o: jnp.ndarray) -> jnp.ndarray:
    """
    ...
    xi_vec = jnp.exp(gamma * (jnp.arange(chunk_size, dtype=jnp.float32) + 1))
    zeta_vec = jnp.exp(gamma * (chunk_size - 1 - jnp.arange(chunk_size, dtype=jnp.float32)))
    ...
    def scan_body_chunkwise_rl(carry_R_in, x_chunk):
        ...
        D_i = create_decay_matrix_rl_jax(gamma, chunk_size, dones_i) # Use JIT-friendly version
        inner_qk_i = (q_i @ k_i_scaled.T) * D_i
        inner_output_i = inner_qk_i @ v_i
        cross_qr_i = q_i @ carry_R_in
        cross_mask_i = create_cross_chunk_mask_rl_jax(chunk_size, dones_i)
        cross_output_i = cross_qr_i * xi_vec[:, None] * cross_mask_i[:, None]
        chunk_output = inner_output_i + cross_output_i

        done_indices_in_chunk = jnp.where(dones_i, jnp.arange(chunk_size), -1)
        last_done_index = jnp.max(done_indices_in_chunk)
        start_j = last_done_index + 1
        last_ep_mask = (jnp.arange(chunk_size) >= start_j).astype(jnp.float32)
...
EMBED_DIM = 4
CHUNK_SIZE = 8
GAMMA = jnp.log(0.9).astype(jnp.float32) # Ensure gamma is float32
...
dones_np = np.zeros(SEQ_LEN, dtype=bool)
dones_np[5] = True
dones_np[15] = True # End of chunk 1
dones_np[25] = True
...
recurrent_retention_rl_jax_jit = jax.jit(recurrent_retention_rl_jax)
chunkwise_retention_rl_jax_jit = jax.jit(chunkwise_retention_rl_jax, static_argnums=(5,)) # chunk_size is static

print("Running Recurrent Retention (RL JAX JIT)...")
recurrent_output_rl = recurrent_retention_rl_jax_jit(Q_data, K_data, V_data, dones_data, GAMMA, W_g_data, W_o_data).block_until_ready()

print("\nRunning Chunkwise Retention (RL JAX JIT)...")
chunkwise_output_rl = chunkwise_retention_rl_jax_jit(Q_data, K_data, V_data, dones_data, GAMMA, CHUNK_SIZE, W_g_data, W_o_data).block_until_ready()
...
are_close_rl = jnp.allclose(recurrent_output_rl, chunkwise_output_rl, atol=1e-5)
