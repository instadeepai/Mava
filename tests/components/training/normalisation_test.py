...
def test_update_and_normalize_observations() -> None:
    ...
    for (x1, x2, x3) in [
        ...
        axes = tuple([slice(0, 15)])
        obs = OLT(observation=jnp.array(x1), legal_actions=[1])
        stats, _ = update_and_normalize_observations(stats, obs, axes=axes)
        obs = OLT(observation=jnp.array(x2), legal_actions=[1])
        stats, _ = update_and_normalize_observations(stats, obs, axes=axes)
        obs = OLT(observation=jnp.array(x3), legal_actions=[1])
        stats, _ = update_and_normalize_observations(stats, obs, axes=axes)
...
def test_normalize_observations() -> None:
    ...
    obs = OLT(observation=x, legal_actions=[1])
    stats = dict(
        mean=jnp.array([0.2]),
        var=jnp.array([4]),
        count=jnp.array([15]),
