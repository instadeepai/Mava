...
initial_parameters_trainer = {
    ...
    "policy_opt_state-network_agent_0": {"opt_state": EmptyState()},
    "policy_opt_state-network_agent_1": {"opt_state": EmptyState()},
    "policy_opt_state-network_agent_2": {"opt_state": EmptyState()},
    "critic_opt_state-network_agent_0": {"opt_state": EmptyState()},
    "critic_opt_state-network_agent_1": {"opt_state": EmptyState()},
    "critic_opt_state-network_agent_2": {"opt_state": EmptyState()},
    "trainer_steps": np.array(0, dtype=np.int32),
    "trainer_walltime": np.array(0.0, dtype=np.float32),
    "evaluator_steps": np.array(0, dtype=np.int32),
    "evaluator_episodes": np.array(0, dtype=np.int32),
...
def mock_builder_with_parameter_client() -> Builder:
    ...
    for net_key in builder.store.networks["networks"].keys():
        builder.store.policy_opt_states[net_key] = {"opt_state": EmptyState()}
    builder.store.critic_opt_states = {}
    for net_key in builder.store.networks["networks"].keys():
        builder.store.critic_opt_states[net_key] = {"opt_state": EmptyState()}
