...
build_venv:
	python3 -m venv mava_30
	mava_30/bin/pip install -r jax_distribution/requirements.txt
	mava_30/bin/pip install jax==0.4.9
	mava_30/bin/pip install jaxlib==0.4.7+cuda11.cudnn82  -f https://storage.googleapis.com/jax-releases/jax_cuda_releases.html
	echo 'export PYTHONPATH="./"' >> mava_30/bin/activate

build_tpu_venv:
	python3 -m venv mava_30
	mava_30/bin/pip install -r jax_distribution/requirements.txt
	mava_30/bin/pip install -U "jax[tpu]" -f https://storage.googleapis.com/jax-releases/libtpu_releases.html
	echo 'export PYTHONPATH="./"' >> mava_30/bin/activate
