...
from mava.systems.tf.offline.madqn import OfflineMADQN, make_default_networks
...
flatland_env_config: Dict = {
    ...
    "obs_builder_object": TreeObsForRailEnv(max_depth=2, predictor=ShortestPathPredictorForRailEnv(max_depth=30)),
}
...
def main(_: Any) -> None:
    ...
    # distributed program
    program = OfflineMADQN(
        environment_factory=environment_factory,
        network_factory=network_factory,
        logger_factory=logger_factory,
        optimizer=snt.optimizers.Adam(learning_rate=1e-4),
        checkpoint_subpath=checkpoint_dir,
        reverb_checkpoint_dir="tfrecords"
    ).build()
