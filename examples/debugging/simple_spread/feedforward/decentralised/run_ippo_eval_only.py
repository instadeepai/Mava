...
"""Example running IPPO on debug MPE environments, only using evaluation (no execution/actors/learning)"""
import functools
...
def main(_: Any) -> None:
    ...
    system.build(
        ...
        epoch_batch_size=5,
        num_epochs=15,
        # no executors - only eval.
        num_executors=0,
        multi_process=True,
        clip_value=False,
        # does only 1 eval and terminates
        evaluation_interval={"evaluator_steps": 0},
        evaluation_duration={"evaluator_episodes": 1},
        termination_condition={"evaluator_episodes": 1},
