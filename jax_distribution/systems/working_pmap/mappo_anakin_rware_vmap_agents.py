...
def run_experiment(env, config):
    ...
    init_y = init_x
    # Obs for all the agents.
    init_x = jax.tree_util.tree_map(lambda x: x[None, ...], init_x)
    # select only obs for a single agent.
    init_y = init_y._replace(step_count=jnp.repeat(init_y.step_count, num_agents))
    init_y = jax.tree_util.tree_map(lambda x: x[0], init_y)
    init_y = jax.tree_util.tree_map(lambda x: x[None, ...], init_y)
    params = network.init(rng_p, init_x, init_y)
