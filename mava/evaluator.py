...
SpecialEvalFn: TypeAlias = Callable[[FrozenDict, PRNGKey, ActorState], Tuple[Metrics, Metrics]]
...
def get_eval_fn(
    ...
    def eval_fn(params: FrozenDict, key: PRNGKey, init_act_state: ActorState) -> Metrics:
        ...
        def _episode(key: PRNGKey, _: Any) -> Tuple[PRNGKey, Metrics]:
            ...
            _, timesteps = jax.lax.scan(_env_step, step_state, jnp.arange(100))
...
def get_while_eval_fn(
    ...
    def eval_fn(
        ...
        def _env_step(eval_state: _EvalEnvStepState) -> _EvalEnvStepState:
            """Performs a single environment step"""
            env_state, ts, key, actor_state = eval_state

            key, act_key = jax.random.split(key)
            # add dummy env dim to ts and actor state
            (_ts, _actor_state) = jax.tree.map(lambda x: x[jnp.newaxis, ...], (ts, actor_state))
            action, actor_state = act_fn(params, _ts, act_key, _actor_state)
            env_state, ts = env.step(env_state, action.squeeze(0))

            return (env_state, ts, key, jax.tree.map(lambda x: x[0], actor_state))
        ...
        def _episode(key: PRNGKey, _: Any) -> Tuple[PRNGKey, Tuple[Metrics, Metrics]]:
            """Simulates `num_envs` episodes."""
            key, reset_key = jax.random.split(key)
            reset_keys = jax.random.split(reset_key, n_vmapped_envs)
            env_state, ts = jax.vmap(env.reset)(reset_keys)

            key, vmap_key = jax.random.split(key)
            vmap_keys = jax.random.split(vmap_key, n_vmapped_envs)
            step_state = env_state, ts, vmap_keys, init_act_state
            (_, timesteps, _, _) = jax.vmap(partial(jax.lax.while_loop, not_done, _env_step))(
                step_state
            )

            metrics = timesteps.extras["episode_metrics"]
            special_metrics = timesteps.extras["env_info"]
            if config.env.log_win_rate:
                metrics["won_episode"] = timesteps.extras["won_episode"]
            ...
            return key, (metrics, special_metrics)

        # This loop is important because we don't want too many parallel envs.
        # So in evaluation we have num_envs parallel envs and loop enough times
        # so that we do at least `eval_episodes` number of episodes.
        _, (metrics, special_metrics) = jax.lax.scan(_episode, key, xs=None, length=episode_loops)
        (metrics, special_metrics) = tree.map(
            lambda x: x.reshape(-1), (metrics, special_metrics)
        )  # flatten metrics
        _ = special_metrics.pop("discount")

        # remove "Achievements/" string from all special metrics keys
        special_metrics = {k.split("/")[1]: v for k, v in special_metrics.items()}

        return metrics, special_metrics
    ...
    def timed_eval_fn(
        ...
        metrics, special_metrics = jax.pmap(eval_fn)(params, key, init_act_state)
        metrics = jax.block_until_ready(metrics)
        special_metrics = jax.block_until_ready(special_metrics)
