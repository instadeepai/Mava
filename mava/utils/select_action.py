...
def ff_sample_actor_output(
    actor, key: PRNGKey, traj_actions=None, get_log_prob=False, action_type="continuos"
) -> Array:
    def get_continuous_act() -> Any:
        actor_mean_logits, actor_log_std_logits = actor
        ...
        log_prob -= jnp.sum(jnp.log(0.5 * (1 - jnp.tanh(actions) ** 2) + 1e-6), axis=-1)
        return (actor_policy, log_prob) if get_log_prob else (actions, log_prob)

    def get_discrete_act() -> Any:
        if get_log_prob:
            log_prob = actor.log_prob(traj_actions)
            return actor, log_prob

        else:
            actions, log_prob = actor.sample_and_log_prob(seed=key)
