...
"""Adapted Implementation of a multi-agents WaterWorld task from EvoJAX.
...
def create_bubbles(key: jnp.ndarray, is_agent: bool) -> BubbleStatus:
    ...
    if is_agent:
        ...
        vel_x, vel_y = random.uniform(k_vel, shape=(2,), minval=-2.5, maxval=2.5)
...
def get_reward(
    agent: BubbleStatus, items: BubbleStatus
) -> Tuple[BubbleStatus, BubbleStatus, jnp.float32]:
    dist = jnp.sqrt(
        jnp.square(agent.pos_x - items.pos_x) + jnp.square(agent.pos_y - items.pos_y)
    )
    rewards = (
        jnp.where(items.bubble_type == TYPE_FOOD, 1.0, -1.0)
        * items.valid
        * jnp.where(dist < MIN_DIST, 1, 0)
    )
    poison_cnt = jnp.sum(jnp.where(rewards == -1.0, 1, 0)) + agent.poison_cnt
...
@partial(jax.vmap, in_axes=(0, None))
def get_rewards(
    agents: BubbleStatus, items: BubbleStatus
) -> Tuple[BubbleStatus, BubbleStatus, jnp.ndarray]:
...
@jax.vmap
def update_agent_state(agent: BubbleStatus, direction: jnp.int32) -> BubbleStatus:
    vel_x = agent.vel_x
...
def get_line_seg_intersection(
    ...
    ua = jnp.where(
        jnp.isclose(denominator, 0.0),
        0,
        ((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3)) / denominator,
    )
    mask1 = jnp.bitwise_and(ua > 0.0, ua < 1.0)
    ub = jnp.where(
        jnp.isclose(denominator, 0.0),
        0,
        ((x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3)) / denominator,
    )
    mask2 = jnp.bitwise_and(ub > 0.0, ub < 1.0)
...
def get_line_dot_intersection(
    ...
    ua = jnp.where(
        jnp.abs(x2 - x1) > jnp.abs(y2 - y1),
        (up[0] - x1) / (x2 - x1),
        (up[1] - y1) / (y2 - y1),
    )
    ua = jnp.where(d > BUBBLE_RADIUS, 0, ua)
    intersected = jnp.bitwise_and(ua > 0.0, ua < 1.0)
    return intersected, up


@partial(jax.vmap, in_axes=(0, None, None, None))
def get_obs(
    agent: BubbleStatus, agents: BubbleStatus, items: BubbleStatus, walls: jnp.ndarray
) -> jnp.ndarray:
    sensor_obs = []
    ...
    for i in jnp.arange(NUM_RANGE_SENSORS):
        ...
        range_xy = jnp.array(
            [
                agent.pos_x + MAX_RANGE * jnp.cos(ang),
        ...
        intersected_with_walls, wall_intersections = get_line_seg_intersection(
            jnp.ones(4) * agent_xy[0],
            jnp.ones(4) * agent_xy[1],
            jnp.ones(4) * range_xy[0],
            jnp.ones(4) * range_xy[1],
            walls[:, 0],
            walls[:, 1],
            walls[:, 2],
            walls[:, 3],
        ...
        dist_to_walls = jnp.where(
            ...
            jnp.sqrt(
                jnp.square(wall_intersections[:, 1] - agent_xy[1])
        ...
        items_and_agents = tree_util.tree_map(
            lambda x, y: jnp.concatenate([x, y], axis=0), items, agents
        )
        n_items = len(items_and_agents.valid)
        intersected_with_items, item_intersections = get_line_dot_intersection(
            jnp.ones(n_items) * agent_xy[0],
            jnp.ones(n_items) * agent_xy[1],
            jnp.ones(n_items) * range_xy[0],
            jnp.ones(n_items) * range_xy[1],
            items_and_agents.pos_x,
            items_and_agents.pos_y,
        )
        dist_to_item = jnp.where(
            jnp.bitwise_and(items_and_agents.valid, intersected_with_items),
            jnp.sqrt(
                jnp.square(item_intersections[:, 1] - agent_xy[1])
                + jnp.square(item_intersections[:, 0] - agent_xy[0])
            ),
            MAX_RANGE,
        )
        dist_to_item = jnp.where(dist_to_item == 0.0, MAX_RANGE, dist_to_item)
        ...
        detected_xy = jnp.where(
            ...
            jnp.where(
                ...
                jnp.array(
                    [dist_to_walls[ix_walls], MAX_RANGE, MAX_RANGE, MAX_RANGE, 0, 0]
                ),
                jnp.array([MAX_RANGE, MAX_RANGE, MAX_RANGE, MAX_RANGE, 0, 0]),
            ),
            jnp.where(
                ...
                jnp.where(
                    ...
                    jnp.array(
                        [
                            ...
                            MAX_RANGE,
                            MAX_RANGE,
                            items.vel_x[ix_items],
                            items.vel_y[ix_items],
                        ]
                    ),
                    jnp.where(
...
class MultiAgentWaterWorld(VectorizedTask):
    ...
    def __init__(
        ...
        self.obs_shape = tuple(
            [
                num_agents,
                NUM_RANGE_SENSORS * SENSOR_DATA_DIM + 2,
            ]
        )
    ...
    def render(state: State) -> Image:
        ...
        sensor_data = np.array(
            state.obs[:, :-2].reshape(-1, NUM_RANGE_SENSORS, SENSOR_DATA_DIM)
        )
        ...
        for v, t, x, y in zip(
            np.array(items.valid, dtype=bool),
