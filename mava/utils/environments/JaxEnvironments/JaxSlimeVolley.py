...
"""Adapted Implementation of the Slime Volley environment in EvoJAX
...
class State(TaskState):
    ...
    obs_right: jnp.ndarray
    obs_left: jnp.ndarray
    steps: jnp.int32
    key: jnp.ndarray
...
def update_state(
    ...
    return updated_game_state, reward_right, reward_left, obs_right, obs_left
...
def get_right_obs(game_state: GameState):
    game = Game(game_state)
    return game.agent_right.getObservation()


def get_left_obs(game_state: GameState):
    game = Game(game_state)
...
class SlimeVolley(VectorizedTask):
    ...
    def __init__(self, max_steps: int = 3000):
        ...
        def reset_fn(key):
            ...
            return State(
                ...
                obs_right=get_right_obs(game_state),
                obs_left=get_left_obs(game_state),
        ...
        def step_fn(state, action_right, action_left=None):
            next_key, key = random.split(state.key)
            cur_state, reward_right, reward_left, obs_right, obs_left = update_state(
    ...
    def step(
        self, state: State, right_action: jnp.ndarray, left_action: jnp.ndarray = None
