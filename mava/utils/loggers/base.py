...
class Logger(MavaLogger):
    def __init__(
        ...
        **external_logger_kwargs: Any,
        ...
        # Don't modify path if s3 url
        if isinstance(directory, str) and "s3" in directory:
            directory = directory
        else:
        ...
        **external_logger_kwargs: Any,
        ...
        if to_json:
            ...
            if (extra_logger_kwargs is not None) and (
                extra_logger_kwargs["json_path"] is not None
            ):
    ...
    def _path(self, subdir: Optional[str] = None) -> str:
        if isinstance(self._directory, str):
            return f"{self._directory}/{self._time_stamp}/{subdir}/{self._label}"
        else:
            if subdir:
