...
def zeros_like(
    x: Union[np.ndarray, int, float, np.number, Dict[str, Any]]
) -> Union[np.ndarray, int, float, np.number, Dict[str, Any]]:
    ...
    if isinstance(x, (int, float, np.number)):
        ...
        for key, value in x.items():
            zero_dict[key] = np.zeros_like(value)
    else:
        raise ValueError(
...
def final_step_like(step: base.Step, next_observation: types.NestedArray) -> base.Step:
    ...
    zero_action, zero_reward, zero_discount, zero_extras = tree.map_structure(
        zeros_like, (step.actions, step.rewards, step.discounts, step.extras)
    )
...
def calculate_priorities(
    priority_fns: base.PriorityFnMapping, steps: Sequence[base.Step]
) -> Dict[str, float]:
