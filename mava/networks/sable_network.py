...
class EncodeBlock(nn.Module):
    ...
    def chunkwise(self, x: chex.Array, hstate: chex.Array, dones: chex.Array) -> chex.Array:
        ...
        ret, updated_hstate = self.retn.chunkwise(key_n=x, query_n=x, value_n=x, hstate=hstate, dones=dones)
        x = self.ln1(x + ret)
        output = self.ln2(x + self.ffn(x))
        return output, updated_hstate
...
class Encoder(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.obs_encoder = nn.Sequential(
            [
                nn.RMSNorm(),
                nn.Dense(
                    self.embed_dim, kernel_init=orthogonal(jnp.sqrt(2)), use_bias=False
                ),
                nn.gelu,
            ],
        )
    ...
    def recurrent(self, obs: chex.Array, hstate: chex.Array) -> Tuple[chex.Array, chex.Array, chex.Array]:
    ...
    def chunkwise(self, obs: chex.Array, hstate: chex.Array, dones: chex.Array)-> Tuple[chex.Array, chex.Array, chex.Array]:
...
class DecodeBlock(nn.Module):
    ...
    def chunkwise(
        ...
        ret, hs1_new = self.retn1.chunkwise(
            key=x, query=x, value=x, hstate=hs1, dones=dones
        )
        ret = self.ln1(x + ret)
...
class Decoder(nn.Module):
    ...
    def __call__(
        self, action: chex.Array, obs_rep: chex.Array) -> chex.Array:
    ...
    def recurrent(
        ...
        for i, block in enumerate(self.blocks):
            ...
            updated_hstates = jax.tree.map(lambda x, y: x.at[:, :, i].set(y), updated_hstates, hs_new)
...
class SableNetwork(nn.Module):
    ...
    def setup(self) -> None:
        ...
        assert self.action_space_type in ["discrete", "continuous"], "Invalid action space type"
    ...
    def get_actions(
        ...
        decayed_hstates = jax.tree.map(
            lambda x: x * self.decay_kappa[None, :, None, None, None], decayed_hstates
        )
        # Apply the encoder
        ...
        return output_actions, output_actions_log, v_loc, None, (updated_enc_hs, updated_dec_hs)
...
def discrete_parallel_act(
    ...
    shifted_action = jnp.zeros((batch_size, sequence_size, action_dim + 1))  
    ...
    shifted_action = shifted_action.at[:, :, 1:].set(one_hot_action)
    shifted_action = jnp.roll(shifted_action, shift=1, axis=1)
...
def discrete_autoregressive_act(
    ...
    # Set the start-of-timestep token (first action as a "start" signal)
    shifted_action = jnp.zeros(action_dim + 1).at[0].set(1)
