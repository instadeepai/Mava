...
class RecQNetwork(nn.Module):
    ...
    def __call__(
        self,
        hidden_state: chex.Array,
        observation_term: RNNObservation,
    ) -> Array:
...
def init(
    ...
    init_hstate = ScannedRNN.initialize_carry(
        (1, cfg.system.n_envs, num_agents), hidden_size
    )  # (1,n_envs, A,x)
...
def make_update_fns(
    ...
    def greedy(hidden_state, nw_params, obs, terms) -> Tuple[chex.Array, chex.Array]:
        ...
        q_vals_masked = jnp.where(
            obs.action_mask, q_values, jnp.zeros(q_values.shape) - 1000
    ...
    def make_transition(timestep, obs: Observation, action) -> Transition:
        ...
        transition = Transition(obs, action, rewards, terms, next_obs, truncs)
    ...
    def select_step_store(
        ...
        new_selection_params, action = select_action(
            selection_params, obs
        )  # should update hidden state, key
        ...
        new_selection_params = ActionSelectionParams(
            new_selection_params.nw_params,
            new_selection_params.hidden_state,
            ~(timestep.discount).astype(bool),
            new_selection_params.t,
            new_selection_params.key,
        )

        new_interact_state = InteractionParams(
            new_selection_params, new_env_state, new_buffer_state, new_obs
        )
    ...
    def scan_apply(params, obs: Observation, terms):
        ...
        hidden_state = ScannedRNN.initialize_carry(
            (cfg.system.batch_size, obs.agents_view.shape[2]), hidden_size
        )
        obs = BT_to_TB(obs)
        ...
        new_hidden_state, next_q_vals_online = q.apply(
            params, hidden_state, obs_terms
        )  # TODO: DOES THIS SCAN??
    ...
    def update_q(
        ...
        first_reward = data.reward[..., jnp.newaxis]  # grab agent zeros assumes same for all agents
        first_term = data.term[..., jnp.newaxis]  # grab agent zeros
        # first_term = data_first.term[...,jnp.newaxis]

        # scan over each sample
        next_q_vals_online = scan_apply(params.online, data.next_obs, data.term)
        next_q_vals_online = jnp.where(
            data.next_obs.action_mask,
            next_q_vals_online,
            jnp.zeros(next_q_vals_online.shape) - 9999999,
        )  # TODO: action masking in nw

        next_q_vals_target = scan_apply(params.target, data.next_obs, data.term)
        ...
        q_grads, q_loss_info = q_grad_fn(
            params.online,
            data.obs,  # is scanned over
            data.term,  # is scanned over
            data.action,  # should only be final value in each traj, B,A (adds axis in fn)
        ...
        new_target_q_params = optax.periodic_update(
            new_online_q_params, params.target, t_train, cfg.system.update_period
        )
    ...
    def train(
        ...
        # learn
        new_params, new_opt_states, q_loss_info, t_train_ = update_q(
    ...
    scanned_interact = lambda state: lax.scan(
        select_step_store, state, None, length=cfg.system.rollout_length
    )
    scanned_train = lambda state: lax.scan(train, state, None, length=cfg.system.epochs)

    # interact and train
    def update_step(
        carry: RNNQLearnerState, _: Any
    ) -> Tuple[RNNQLearnerState, Tuple[Metrics, Metrics]]:
        ...
        selection_params = ActionSelectionParams(
            params.online, hidden_state, terms, t, interact_key
        )
        interact_carry = InteractionParams(selection_params, env_state, buffer_state, obs)
        (
            (_, hidden_state_, terms_, _, _),
            env_state_,
            buffer_state_,
            obs_,
        ), metrics = scanned_interact(interact_carry)
