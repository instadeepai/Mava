...
def make_update_fns(
    ...
    def update_q(
        ...
        data_first: Dict[str, chex.Array] = jax.tree_map(lambda x: x[:, :-1, ...], data) # (B, T, ...)
        ...
        # Eps defaults to 0
        # NOTE (Ruan): Manually masking like we do in the commented out code works. 
        # but calling .mode() on the distribution does not work. I have no idea why.
        # NOTE (Ruan): Does the dist automaticall have action masking?

        # # #####################################
        # # manual next action call and manually masking of q_values
        # # Scan over each sample
        # hidden_state, next_obs_term_or_trunc = prep_inputs_to_scannedrnn(
        #     data.obs, data.term_or_trunc,
        # ) # (T, B, ...)
        # _, next_q_vals_online = q_net.apply(
        #     params.online, hidden_state, next_obs_term_or_trunc, method="get_q_values"
        # )
        # next_q_vals_online = switch_leading_axes(next_q_vals_online)  # (T, B, ...) -> (B, T, ...)
        ...
        # jax.debug.print("Dist and Manual Next Action Match: {x}", x=jnp.all(next_action == next_action_dist))
