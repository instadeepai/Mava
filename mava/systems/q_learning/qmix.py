...
from mava.networks import QMixNetwork, RecQNetwork, ScannedRNN
...
def init(
    ...
    q_mixer = hydra.utils.instantiate(
        cfg.network.q_mixer, num_actions=action_dim, num_agents=num_agents
    )
...
def make_update_fns(
    ...
    def prep_inputs_to_scannedrnn(
        obs: ObservationGlobalState, term_or_trunc: chex.Array
    ) -> chex.Array:
    ...
    def update_q(
        ...
        q_grads, q_loss_info = q_grad_fn(
            (params.online, params.mixer_online), obs, term_or_trunc, action, target_q_val
        )
...
def run_experiment(cfg: DictConfig) -> float:
    ...
    (env, eval_env), q_net, mixer, opt, rb, learner_state, logger, key = init(cfg)
    update = make_update_fns(cfg, env, q_net, mixer, opt, rb)
...
@hydra.main(config_path="../../configs", config_name="default_qmix.yaml", version_base="1.2")
