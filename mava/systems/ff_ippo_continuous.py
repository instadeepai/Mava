...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, PPOTransition]:
            ...
            action, log_prob = ff_sample_actor_output(actor_policy, policy_key)
            ...
            # ? What is the difference between `raw_action` and `action` in this case?
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    actor_policy, log_prob = ff_sample_actor_output(
                        actor_policy, None, traj_batch.action, True
                    )
...
def learner_setup(
    ...
    config.system.num_actions = int(env.action_spec().shape[1])
    ...
    actor_network, critic_network = networks.make(
        config=config, network="continuousff", centralised_critic=False
    )
