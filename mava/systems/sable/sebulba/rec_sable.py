...
from mava.systems.ppo.types import RNNPPOTransition as Transition
...
def get_learner_step_fn(
    ...
    def _update_step(
        ...
        prev_hstates = jax.tree.map(lambda x: x[0], traj_batch.hstates)
        updated_hstates = jax.tree.map(lambda x: x[-1], traj_batch.hstates)
