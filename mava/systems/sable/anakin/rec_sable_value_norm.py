...
class NormParams(NamedTuple):
    value_norm_params: ValueNormParams = ValueNormParams()
    reward_norm_params: RewardMoments = RewardMoments()
...
def _get_running_mean_var(value_norm_state: ValueNormParams) -> Tuple[chex.Array, chex.Array]:
...
def update_running_mean_var(
    value_norm_state: ValueNormParams, input_batch: chex.Array
) -> ValueNormParams:
    # TODO: Set axes later on.
    ...
    new_running_mean_sq = value_norm_state.running_mean_sq * value_norm_state.beta + (
...
def normalise(value_norm_state: ValueNormParams, values: chex.Array) -> chex.Array:
    debiased_mean, debiased_var = _get_running_mean_var(value_norm_state)
    values = (values - debiased_mean) / jnp.sqrt(debiased_var)
    return values


def denormalise(value_norm_state: ValueNormParams, values: chex.Array) -> chex.Array:
    debiased_mean, debiased_var = _get_running_mean_var(value_norm_state)
...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(learner_state: LearnerState, _: int) -> Tuple[LearnerState, Transition]:
            ...
            params, opt_states, key, env_state, last_timestep, hstates, norm_params = learner_state
            ...
            # batch_mean = jnp.mean(timestep.reward[:, 0])
            # batch_std = jnp.std(timestep.reward[:, 0])
            ...
            learner_state = LearnerState(
                params, opt_states, key, env_state, timestep, hstates, norm_params
            )
        ...
        # CALCULATE ADVANTAGE
        params, opt_states, key, env_state, last_timestep, updated_hstates, norm_params = (
            learner_state
        )
        ...
        def _calculate_gae(
            ...
            def _get_advantages(
                ...
                delta = (
                    reward
                    + gamma * denormalise(value_norm_state, next_value) * (1 - next_done)
                    - denormalise(value_norm_state, value)
                )
            ...
            return advantages, advantages + denormalise(value_norm_state, traj_batch.value)

        advantages, targets = _calculate_gae(
            traj_batch, current_val, current_done, norm_params.value_norm_params
        )
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                params, opt_state, key, value_norm_params = train_state
                ...
                def _loss_fn(
                    ...
                    # Normalise value targets to regress to normalised values
                    value_targets = normalise(value_norm_params, value_targets)
                ...
                return (new_params, new_opt_state, key, value_norm_params), loss_info
            ...
            # UPDATE MINIBATCHES
            (params, opt_states, entropy_key, _), loss_info = jax.lax.scan(
                _update_minibatch,
                (params, opt_states, entropy_key, norm_params.value_norm_params),
        ...
        # After all the epochs update the mean and variance of the value function with traj_batch values
        new_value_norm_params = update_running_mean_var(
            norm_params.value_norm_params, traj_batch.value
        )

        new_norm_params = NormParams(
            value_norm_params=new_value_norm_params,
            reward_norm_params=norm_params.reward_norm_params,
        )
...
def learner_setup(
    ...
    norm_params = jax.tree_map(broadcast_scalar, norm_params)

    # Duplicate learner across devices.
    replicate_learner = flax.jax_utils.replicate(replicate_learner, devices=jax.devices())
    init_hstates = flax.jax_utils.replicate(init_hstates, devices=jax.devices())
    norm_params = flax.jax_utils.replicate(norm_params, devices=jax.devices())
