...
LearnerStateWithStartState = namedtuple(
    "LearnerStateWithStartState", ["learner_state", "start_state"]
)


def make_env(config: DictConfig) -> Tuple[MarlEnv, MarlEnv]:
...
def get_learner_fn(
    ...
    def _update_step(
        learner_state_with_learnable_instances: Tuple[LearnerState, EnvInstance], _: Any
    ) -> Tuple[Tuple[LearnerState, EnvInstance], Tuple]:
        ...
        sampled_idxs = jax.random.choice(
            sampled_key_0,
            jnp.arange(config.ued.batch_size * config.ued.num_batches),
    ...
    def learner_fn(
        learner_state_with_learnable_instances: Tuple[LearnerState, EnvInstance],
...
def get_learnability_set(
    ...
    config: DictConfig,
    env: JaxMarlWrapper,
) -> Tuple[chex.Array, EnvInstance]:
    def _batch_step(_: None, rng: chex.PRNGKey) -> Tuple[None, Tuple[chex.Array, EnvInstance]]:
        def _env_step(runner_state: Tuple, _: Any) -> Tuple[Tuple, SFLTransition]:
            """Step the environment."""
            ...
            transition = SFLTransition(done, goal_reached)
            runner_state = (rng, env_state, timestep.observation, start_state)
            return runner_state, transition

        @partial(jax.vmap, in_axes=(None, 1, 1))
        @partial(jax.jit, static_argnums=(0,))
        def _calc_outcomes_by_agent(
            max_steps: int, dones: chex.Array, goal_reached: chex.Array
        ) -> dict[str, chex.Array]:
            idxs = jnp.arange(max_steps)

            @partial(jax.vmap, in_axes=(0, 0))
            def __ep_outcomes(start_idx: chex.Array, end_idx: chex.Array) -> chex.Array:
            ...
            success = __ep_outcomes(jnp.concatenate([jnp.array([-1]), done_idxs[:-1]]), done_idxs)
        ...
        print("o", o)
        success_by_env = o["success_rate"].reshape((env.num_agents, config.ued.batch_size))
...
def test_get_learnability_set(_config: DictConfig) -> None:
    ...
    learnability, top_1000_instances = get_learnability_set(
