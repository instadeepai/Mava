...
def learner_setup(
    ...
    env = environments.make_smac_env(config, config.arch.num_envs)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    evaluator, evaluator_envs = get_eval_fn(
        environments.make_smac_env, eval_act_fn, config, np_rng, absolute_metric=False
    )
        ...
        for thread_id in range(config.arch.n_threads_per_executor):
            ...
            actor = threading.Thread(
                ...
                args=(
                    ...
                    environments.make_smac_env(config, config.arch.num_envs),
                    config,
                    pipe,
                    params_source,
                    apply_fns,
    ...
    if config.arch.absolute_metric:
        ...
        abs_metric_evaluator, abs_metric_evaluator_envs = get_eval_fn(
            environments.make_smac_env, eval_act_fn, config, np_rng, absolute_metric=True
        )
        key, eval_key = jax.random.split(key, 2)
