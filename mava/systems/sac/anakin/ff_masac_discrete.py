...
from mava.networks import FeedForwardDiscreteQNet as QNetwork
...
def init(
    ...
    act_single = acts[jnp.newaxis, 0]  # single agents action
    ...
    q_network = QNetwork(critic_torso, num_actions=env.action_dim, centralised_critic=True)
    q1_params = q_network.init(q1_key, obs_single_batched)
    q2_params = q_network.init(q2_key, obs_single_batched)
    q1_target_params = q_network.init(q1_target_key, obs_single_batched)
    q2_target_params = q_network.init(q2_target_key, obs_single_batched)
    ...
    # making sure we have shape=(1, N, 1) so broacasting works fine
...
def make_update_fns(
    ...
    def q_loss_fn(
        ...
        q1_a_values = q_net.apply(q1_params, obs)
        q2_a_values = q_net.apply(q2_params, obs)
        # DQN style Q-val selection
        q1_a_val = jnp.take_along_axis(q1_a_values, action[..., jnp.newaxis], axis=-1).squeeze(-1)
        q2_a_val = jnp.take_along_axis(q2_a_values, action[..., jnp.newaxis], axis=-1).squeeze(-1)

        # q1_a_values = q_net.apply(q1_params, obs, joint_action)
        # q2_a_values = q_net.apply(q2_params, obs, joint_action)

        q1_loss = jnp.mean(jnp.square(q1_a_val - target))
        q2_loss = jnp.mean(jnp.square(q2_a_val - target))
    ...
    def update_q(
        ...
        next_q1_val = q_net.apply(params.q.targets.q1, data.next_obs)
        next_q2_val = q_net.apply(params.q.targets.q2, data.next_obs)
        next_q_val = jnp.minimum(next_q1_val, next_q2_val)
        next_q_val = next_q_val - jnp.exp(params.log_alpha) * next_action_log_probs
        # TODO: why is this needed!? Stoix + cleanRL have it
        next_q_val = (next_action_probs * next_q_val).sum(axis=-1)
    ...
    def explore(carry: LearnerState, _: Any) -> Tuple[LearnerState, Metrics]:
        ...
        action = jax.random.randint(explore_key, full_action_shape, 0, 6)  # todo: grab true min/max
        next_obs, env_state, buffer_state, metrics = step(action, obs, env_state, buffer_state)
