...
def init(
    ...
    -------
        Tuple containing:
            ...
            RecLearnerState: The initial learner state.
    ...
    joint_acts_batched = joint_acts[jnp.newaxis, jnp.newaxis, ...]  # batch + time of joint acts
    joint_acts_batched = jnp.repeat(joint_acts_batched[:, :, jnp.newaxis], n_agents, 2)
    obs = env.observation_spec().generate_value()
    obs_single_batched = tree.map(lambda x: x[jnp.newaxis, jnp.newaxis, ...], obs)
    done = jnp.zeros((1, 1, n_agents))
    ...
    actor_action_head = hydra.utils.instantiate(cfg.network.action_head, action_dim=env.action_dim)
    ...
    init_hstate = ScannedRNN.initialize_carry((1, n_agents), cfg.network.hidden_state_dim)
    actor_params = actor_network.init(actor_key, init_hstate, (obs_single_batched, done))
    ...
    q1_params = q_network.init(q1_key, init_hstate, obs_single_batched, joint_acts_batched, done)
    q2_params = q_network.init(q2_key, init_hstate, obs_single_batched, joint_acts_batched, done)
    q1_target_params = q_network.init(
        q1_target_key, init_hstate, obs_single_batched, joint_acts_batched, done
    )
    q2_target_params = q_network.init(
        q2_target_key, init_hstate, obs_single_batched, joint_acts_batched, done
    )
    ...
    rb = fbx.make_trajectory_buffer(
        ...
        sample_sequence_length=cfg.system.sequence_length + 1,
        period=1,  # sample any unique trajectory
        add_batch_size=cfg.arch.num_envs,
        sample_batch_size=cfg.system.batch_size,
        # todo: sort this out
        max_length_time_axis=int(cfg.system.buffer_size),
        min_length_time_axis=int(cfg.system.batch_size),
    )
    ...
    first_dones = jnp.repeat(first_timestep.last()[..., jnp.newaxis], n_agents, -1)

    t = jnp.zeros((cfg.arch.n_devices, cfg.system.update_batch_size), dtype=int)

    hs = jnp.repeat(init_hstate, cfg.arch.num_envs, 0)
    hidden_states = replicate(HiddenStates(hs.copy(), hs.copy(), hs.copy()))

    # Initial learner state.
    learner_state = RecLearnerState(
        first_obs,
        first_dones,
        env_state,
        buffer_state,
...
def make_update_fns(
    ...
    Callable[[RecLearnerState], Tuple[RecLearnerState, Metrics]],
    Callable[[RecLearnerState], Tuple[RecLearnerState, Tuple[Metrics, Metrics]]],
]:
    ...
    def act(carry: ActCarry, _: Any) -> Tuple[ActCarry, Dict]:
        ...
        ac_in = jax.tree.map(lambda x: x[jnp.newaxis], (obs, dones))  # add time dim
    ...
    def update_step(
        carry: RecLearnerState, _: Any
    ) -> Tuple[RecLearnerState, Tuple[Metrics, Metrics]]:
        """Act, sample, learn. The body of the main SAC loop."""
        obs, done, env_state, buffer_state, params, hs, opt_states, t, key = carry
        key, act_key, learn_key = jax.random.split(key, 3)
        # Act
        act_state = (params.actor, obs, done, hs.actor, env_state, buffer_state, act_key)
        (_, next_obs, dones, act_hs, env_state, buffer_state, _), metrics = scanned_act(act_state)
        hs = hs._replace(actor=act_hs)
...
@hydra.main(config_path="../../configs", config_name="default_rec_masac.yaml", version_base="1.2")
