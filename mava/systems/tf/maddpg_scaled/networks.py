...
def make_default_networks(
    ...
    if isinstance(policy_networks_layer_sizes, Sequence):
        policy_networks_layer_sizes = {
            key: policy_networks_layer_sizes for key in net_spec_config.keys()
        }
    if isinstance(critic_networks_layer_sizes, Sequence):
        critic_networks_layer_sizes = {
            key: critic_networks_layer_sizes for key in net_spec_config.keys()
        }

    observation_networks = {}
    policy_networks = {}
    critic_networks = {}
    for net_key in net_spec_config.keys():
        # TODO (dries): Make specs[key].actions
        #  return a list of specs for hybrid action space
        # Get total number of action dimensions from action spec.
        spec_key = net_spec_config[net_key]
        agent_act_spec = specs[spec_key].actions
        if type(specs[spec_key].actions) == DiscreteArray:
        ...
        if archecture_type == ArchitectureType.feedforward:
            policy_network = [
                networks.LayerNormMLP(
                    policy_networks_layer_sizes[net_key], activate_final=True
                ),
            ]
        elif archecture_type == ArchitectureType.recurrent:
            policy_network = [
                networks.LayerNormMLP(
                    policy_networks_layer_sizes[net_key][:-1], activate_final=True
                ),
                snt.LSTM(policy_networks_layer_sizes[net_key][-1]),
            ]
        ...
        critic_network = snt.Sequential(
            [
                ...
                networks.LayerNormMLP(
                    list(critic_networks_layer_sizes[net_key]) + [1],
                    activate_final=False,
                ),
            ]
        )
        observation_networks[net_key] = observation_network
        policy_networks[net_key] = policy_network
        critic_networks[net_key] = critic_network
