...
from mava.components.tf.modules.stabilising import (
    FingerPrintStabalisation,
    fingerprints,
)
...
from mava.wrappers import DetailedPerAgentStatistics, MADQNDetailedTrainerStatistics
...
class MADQN:
    ...
    def __init__(
        ...
        exploration_scheduler: Type[
            LinearExplorationScheduler
        ] = LinearExplorationScheduler,
        epsilon_min: float = 0.05,
        epsilon_decay: float = 1e-4,
        fingerprint_fn: Optional[Type[FingerPrintStabalisation]] = None,
        ...
        # Get the environment spec if not provided.
    ...
    def _get_extra_specs(self) -> Any:
        ...
        """
        # Dict to hold extra specs.
        extra_specs = {}

        # Maybe get fingerprint spec.
        if self._fingerprint_fn is not None:
            fingerprint_module: FingerPrintStabalisation = self._fingerprint_fn()
            fingerprint_spec = {"fingerprint": fingerprint_module.get_spec()}

            # Add to extra specs.
            extra_specs.update(fingerprint_spec)
    ...
    def replay(self) -> Any:
        ...
        return self._builder.make_replay_tables(self._builder._config.environment_spec)
    ...
    def trainer(
        ...
        networks = self._network_factory(  # type: ignore
            environment_spec=self._builder._config.environment_spec,
            agent_net_keys=self._agent_net_keys,
        )

        # Create system architecture with target networks.
        architecture: DecentralisedValueActor = self._architecture(
            environment_spec=self._builder._config.environment_spec,
            value_networks=networks["q_networks"],
            agent_net_keys=self._agent_net_keys,
        )

        # Maybe apply fingerprinting to architecture.
        if self._fingerprint_fn is not None:
            fingerprint_module: FingerPrintStabalisation = self._fingerprint_fn()
            fingerprint_module.apply_to_architecture(architecture)
        else:
        ...
        # Make the dataset for the trainer to sample from.
        ...
        # Wrap trainer in stats wrapper.
        trainer = MADQNDetailedTrainerStatistics(trainer)
    ...
    def executor(
        ...
        """
        # Create an adder to insert experience into reverb.
        ...
        # Grab the action selector and value network.
        ...
        # Initialise networks and grab the q-networks for execution.
        ...
        # Wrap loop in statistics wrapper.
...
class RecurrentMADQN:
    """Recurrent MADQN system."""
    ...
    def trainer(
        ...
        if self._builder._replay_stabiliser_fn is not None:
            stabilisation_module = self._builder._config._replay_stabiliser_fn(  # type: ignore
                architecture
            )
