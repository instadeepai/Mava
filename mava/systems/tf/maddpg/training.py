...
class DecentralisedRecurrentCommMADDPGTrainer(BaseRecurrentMADDPGTrainer):
    ...
    def _policy_actions_messages(
        ...
        if batch_first:
            actions = {
                agent: tf2_utils.batch_to_sequence(actions[agent])
                for agent in self._agents
            }
    ...
    def _forward(self, inputs: Any) -> None:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            p_actions = self._policy_actions_messages(
    ...
    # Forward pass that calculates loss.
    def _forward_(self, inputs: Any) -> None:
        data = inputs.data
