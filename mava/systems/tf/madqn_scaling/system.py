...
from mava.systems.tf.madqn_scaling import builder, training
from mava.systems.tf.madqn_scaling.execution import (
    MADQNScalingFeedForwardExecutor,
...
class MADQNScaling:
    ...
    def __init__(  # noqa
        ...
        exploration_scheduler_fn: Optional[BaseExplorationScheduler] = LinearExplorationScheduler,
        exploration_scheduler_kwargs: dict = {
            "epsilon_start": 1.0,
            "epsilon_min": 0.05,
            "epsilon_decay": 1e-4
        },
        ...
        self._builder = builder.MADQNScalingBuilder(
            builder.MADQNScalingConfig(
                ...
                exploration_scheduler_kwargs=exploration_scheduler_kwargs,
        ...
        for agent in agents:
            ...
            core_state_specs[agent] = (
                tf2_utils.squeeze_batch_dim(
                    networks["q_networks"][agent_net_key].initial_state(1)
                ),
            )
    ...
    def create_system(
        ...
        system_networks = system.create_system()
        behaviour_networks = system.create_behaviour_policy()
        return behaviour_networks, system_networks, networks["action_selectors"]

    def variable_server(self) -> MavaVariableSource:
        """Create the variable server."""
        # Create the system
        _, networks, action_selectors = self.create_system()
    ...
    def executor(
        ...
        # Create the system
        policy_networks, networks, action_selectors = self.create_system()
    ...
    def evaluator(
        ...
        # Create the system
        behaviour_policy_networks, networks, action_selectors = self.create_system()
