...
from mava.types import OptStates, Params, Observation
...
def rollout(  # noqa: CCR001
    ...
    # Create envs
    envs = make(cfg)(cfg.arch.num_envs)
    ...
    def get_action_and_value(
        ...
        policy = vmap_actor_apply(params.actor_params, Observation(next_obs, actions_mask, None))
        action, logprob = policy.sample_and_log_prob(seed=subkey)

        value = vmap_critic_apply(params.critic_params, Observation(next_obs, actions_mask, None)).squeeze()
...
def get_trainer(cfg: DictConfig, apply_fns: Tuple, optim_updates_fn: Tuple) -> Callable:
    ...
    def compute_gae(
        ...
        next_value = vmap_critic_apply(
            ...
            Observation(next_obs, next_obs, None)
        )
    ...
    def actor_loss(
        ...
        policy = vmap_actor_apply(actor_params, Observation(obs, actions_mask, None))
    ...
    def critic_loss(critic_params: FrozenDict, obs: Array, target_values: Array) -> Tuple:
        """Critic loss."""
        newvalue = vmap_critic_apply(critic_params, Observation(obs, obs, None))
...
def run_experiment(cfg: DictConfig) -> None:  # noqa: CCR001
    """Run experiment."""
    # Logger setup
    log: SebulbaLogFn = logger_setup(cfg)  # type: ignore
    ...
    learner_keys = jax.device_put_replicated(key, cfg.system.learner_devices)
    ...
    dummy_envs = make(cfg)(  # Create dummy_envs to get observation and action spaces
        cfg.arch.num_envs
    )
    ...
    actor, critic = get_networks(config=cfg, network="feedforward", centralised_critic=False)

    actor_params = actor.init(actor_key, Observation(init_obs, init_action_mask, None))
    ...
    vmap_actor_apply = jax.vmap(actor.apply, in_axes=(None, Observation(1,1,None)), out_axes=(1))

    critic_params = critic.init(critic_key, Observation(init_obs, init_action_mask, None))
    ...
    vmap_critic_apply = jax.vmap(critic.apply, in_axes=(None, Observation(1,1,None)), out_axes=(1))
    ...
    # Evaluation setup #TODO: fix this
    ...
    while True:
        ...
        if trainer_update_number % log_frequency == 0:
            # Logging training info #TODO: back to this
            log(
