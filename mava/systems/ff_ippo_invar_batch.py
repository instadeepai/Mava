...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, PPOTransition]:
            ...
            params, opt_states, key, env_state, last_timestep, ewma_moments = learner_state
            ...
            learner_state = LearnerState(params, opt_states, key, env_state, timestep, ewma_moments)
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                params, opt_states, ewma_moments = train_state
                ...
                def _actor_loss_fn(
                    ...
                    prox_policy = actor_apply_fn(prox_actor_params, traj_batch.obs)
                    prox_log_prob = prox_policy.log_prob(traj_batch.action)

                    # CALCULATE ACTOR LOSS
                    ratio = jnp.exp(log_prob - prox_log_prob) * jnp.exp(
                        prox_log_prob - traj_batch.log_prob
                    )
                ...
                w_new = 1.0 + ewma_moments.beta_prox * ewma_moments.w
                scaled_actor_params = optax.tree_utils.tree_scalar_mul(
                    (1.0 / w_new), actor_new_params
                )
                scaled_prox_params = optax.tree_utils.tree_scalar_mul(
                    ewma_moments.beta_prox * (ewma_moments.w / w_new), params.prox_actor_params
                )
                new_prox_actor_params = optax.tree_utils.tree_add(
                    scaled_actor_params, scaled_prox_params
                )
                ...
                new_params = Params(
                    actor_new_params,
                    critic_new_params,
                    new_prox_actor_params,
                    params.prox_critic_params,
                )
                new_opt_state = OptStates(actor_new_opt_state, critic_new_opt_state)

                new_ewma_moments = EWMAMoments(ewma_moments.beta_prox, w_new)
                ...
                return (new_params, new_opt_state, new_ewma_moments), loss_info

            params, opt_states, traj_batch, advantages, targets, key, ewma_moments = update_state
            ...
            # UPDATE MINIBATCHES
            (params, opt_states, ewma_moments), loss_info = jax.lax.scan(
                _update_minibatch, (params, opt_states, ewma_moments), minibatches
            )

            update_state = (params, opt_states, traj_batch, advantages, targets, key, ewma_moments)
        ...
        learner_state = LearnerState(
            params, opt_states, key, env_state, last_timestep, ewma_moments
        )
...
def learner_setup(
    ...
    params = Params(
        ...
        prox_actor_params=copy.deepcopy(actor_params),
        prox_critic_params=copy.deepcopy(critic_params),
    )
    ...
    ewma_moments = EWMAMoments(config.system.beta_prox, 1.0)
    ...
    # Duplicate ewma for update_batch_size.
    broadcast = lambda x: jnp.broadcast_to(x, config.system.update_batch_size)
    ewma_moments = jax.tree_map(broadcast, ewma_moments)

    # Duplicate learner across devices.
    ewma_moments = flax.jax_utils.replicate(ewma_moments, devices=jax.devices())

    # Initialise learner state.
    params, opt_states, step_keys = replicate_learner
    init_learner_state = LearnerState(
        params, opt_states, step_keys, env_states, timesteps, ewma_moments
    )
