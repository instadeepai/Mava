...
class GymToJumanji:
    ...
    def reset(self, seed: Optional[list[int]] = None, options: Optional[dict] = None) -> TimeStep:
        ...
        teminated = np.zeros((num_envs, self.num_agents), dtype=float)
    ...
    def step(self, action: list) -> TimeStep:
        ...
        terminated = np.repeat(terminated[..., np.newaxis], repeats=self.num_agents ,axis=-1) # (B,) --> (B, N)
    ...
    def _format_observation(
        self, obs: NDArray, action_mask: Tuple[NDArray], global_obs: Optional[Tuple[NDArray]] = None
    ...
    def _create_timestep(
        ...
        observation = self._format_observation(obs, info["action_mask"], info.get("global_obs", (None,)))
        # Filter out the masks and auxiliary data #this is beyond ugly
        extras = {}
        extras["episode_metrics"] = {key: value for key, value in info["metrics"].items() if key[0] != "_"}
        extras["real_next_obs"] = self._format_observation(info["real_next_obs"], info["real_next_action_mask"] , info["real_next_global_obs"])
