...
@dataclass
class MAMCTSExecutorSelectActionProcessConfig:
    root_fn: RootFn
...
# TODO
class MAMCTSFeedforwardExecutorSelectAction(Component):
    def __init__(
        self,
        config: MAMCTSExecutorSelectActionProcessConfig = MAMCTSExecutorSelectActionProcessConfig(),
    ):
        """_summary_
    ...
    def on_execution_select_action_compute(self, executor: SystemExecutor) -> None:
        ...
        # We can also try jit over all the agents in a for loop. This would
        # allow the jit function to save us even more time.
        root_output = self.config.root_fn(params, rng_key, None)
        ...
        executor.store.action_info, executor.store.policy_info = (
