...
class MAPGWithTrustRegionStepNorm(MAPGWithTrustRegionStep):
    def __init__(
        self,
        config: MAPGWithTrustRegionStepConfig = MAPGWithTrustRegionStepConfig(),
    ):
        """Component defines the MAPGWithTrustRegion SGD step with values normalization.
    ...
    def on_training_step_fn(self, trainer: SystemTrainer) -> None:
        ...
        def sgd_step(
            ...
            Args:
                states: Training states (network params, optimiser states and state values statisitics).
                sample: Reverb sample.
            ...
            for key in rewards.keys():
                ...
                advantages[key], target_values[key] = batch_gae_advantages(
                    rewards[key], discounts[key], behavior_values[key], jnp.repeat(stats[key][None,:], batch_tmp, axis=0)
                )
            ...
            new_states = TrainingStateStats(
                params=new_params, opt_states=new_opt_states, random_key=new_key, stats=stats
            )
            return new_states, metrics
        ...
        def step(sample: reverb.ReplaySample) -> Tuple[Dict[str, jnp.ndarray]]:
            ...
            states = TrainingStateStats(
                params=params, opt_states=opt_states, random_key=random_key, stats=stats
            )
            ...
            for agent_key in trainer.store.trainer_agent_net_keys.keys():
                trainer.store.stats[agent_key] = new_states.stats[agent_key]
...
class MAPGWithTrustRegionStepSeparateNetworksNorm(MAPGWithTrustRegionStepSeparateNetworks):
    def __init__(
        self,
        config: MAPGWithTrustRegionStepConfig = MAPGWithTrustRegionStepConfig(),
    ):
    ...
    def on_training_step_fn(self, trainer: SystemTrainer) -> None:
        ...
        @jit
        def sgd_step(
            states: TrainingStateStatsSeparateNetworks, sample: reverb.ReplaySample
        ) -> Tuple[TrainingStateSeparateNetworks, Dict[str, jnp.ndarray]]:
            """Performs a minibatch SGD step.
            ...
            for key in rewards.keys():
                batch_tmp = rewards[key].shape[0]
                advantages[key], target_values[key] = batch_gae_advantages(
                    rewards[key], discounts[key], behavior_values[key], np.repeat(stats[key][None,:], batch_tmp, axis=0)
                )
                stats[key] = compute_running_mean_var_count(stats[key], target_values[key])
                target_values[key] = jax.lax.stop_gradient(normalize(stats[key], target_values[key]))
            ...
            new_states = TrainingStateStatsSeparateNetworks(
                policy_params=new_policy_params,
                critic_params=new_critic_params,
                policy_opt_states=new_policy_opt_states,
                critic_opt_states=new_critic_opt_states,
        ...
        def step(sample: reverb.ReplaySample) -> Tuple[Dict[str, jnp.ndarray]]:
            ...
            states = TrainingStateStatsSeparateNetworks(
                policy_params=policy_params,
                critic_params=critic_params,
                policy_opt_states=policy_opt_states,
                critic_opt_states=critic_opt_states,
