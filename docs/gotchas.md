...
- If you encounter `ModuleNotFoundError: No module named 'mava'`, a likely fix is to add the home directroy of the cloned Mava source code to `PYTHONPATH` with `export PYTHONPATH=<mava_homedir>`. On a linux-based system, this command can be added to `.bashrc`, to avoid having to export the path every time a new terminal is opened.
